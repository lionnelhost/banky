...
import { sampleWithRequiredData } from '../canal.test-samples';
...
describe('Canal Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.canals?.[0]).toEqual(expect.objectContaining({ idCanal: 'ABC' }));
  });

  describe('trackIdCanal', () => {
    it('Should forward to canalService', () => {
      const entity = { idCanal: 'ABC' };
      jest.spyOn(service, 'getCanalIdentifier');
      const idCanal = comp.trackIdCanal(0, entity);
      expect(service.getCanalIdentifier).toHaveBeenCalledWith(entity);
      expect(idCanal).toBe(entity.idCanal);
    });
  });
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.query).toHaveBeenLastCalledWith(expect.objectContaining({ sort: ['idCanal,desc'] }));
  });
