...
import { sampleWithRequiredData } from '../agence.test-samples';
...
describe('Agence Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.agences?.[0]).toEqual(expect.objectContaining({ idAgence: 'ABC' }));
  });

  describe('trackIdAgence', () => {
    it('Should forward to agenceService', () => {
      const entity = { idAgence: 'ABC' };
      jest.spyOn(service, 'getAgenceIdentifier');
      const idAgence = comp.trackIdAgence(0, entity);
      expect(service.getAgenceIdentifier).toHaveBeenCalledWith(entity);
      expect(idAgence).toBe(entity.idAgence);
    });
  });
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.query).toHaveBeenLastCalledWith(expect.objectContaining({ sort: ['idAgence,desc'] }));
  });
