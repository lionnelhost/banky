...
describe('Contrat Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.contrats?.[0]).toEqual(expect.objectContaining({ idContrat: 'ABC' }));
  });

  describe('trackIdContrat', () => {
    it('Should forward to contratService', () => {
      const entity = { idContrat: 'ABC' };
      jest.spyOn(service, 'getContratIdentifier');
      const idContrat = comp.trackIdContrat(0, entity);
      expect(service.getContratIdentifier).toHaveBeenCalledWith(entity);
      expect(idContrat).toBe(entity.idContrat);
    });
  });
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.query).toHaveBeenLastCalledWith(expect.objectContaining({ sort: ['idContrat,desc'] }));
  });
