 ...
 */
public class AgenceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all agences").get("/api/agences").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new agence")
                        .post("/api/agences")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"idAgence\": \"SAMPLE_TEXT\"" +
                                ", \"codeAgence\": \"SAMPLE_TEXT\"" +
                                ", \"nomAgence\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_agence_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created agence").get("${new_agence_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created agence").delete("${new_agence_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Agence entity").exec(scn);
