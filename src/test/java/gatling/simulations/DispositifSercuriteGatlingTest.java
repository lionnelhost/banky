...
public class DispositifSercuriteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all dispositifSercurites")
                    .get("/api/dispositif-sercurites")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dispositifSercurite")
                        .post("/api/dispositif-sercurites")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"idCanal\": \"SAMPLE_TEXT\"" +
                                ", \"idTypeTransaction\": \"SAMPLE_TEXT\"" +
                                ", \"idDispositif\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dispositifSercurite_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created dispositifSercurite").get("${new_dispositifSercurite_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created dispositifSercurite").delete("${new_dispositifSercurite_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DispositifSercurite entity").exec(scn);
