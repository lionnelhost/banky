 ...
 */
public class ContratGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all contrats").get("/api/contrats").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new contrat")
                        .post("/api/contrats")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"idContrat\": \"SAMPLE_TEXT\"" + ", \"dateValidite\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_contrat_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created contrat").get("${new_contrat_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created contrat").delete("${new_contrat_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Contrat entity").exec(scn);
