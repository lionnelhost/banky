...
public class JourFerierGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all jourFeriers").get("/api/jour-feriers").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new jourFerier")
                        .post("/api/jour-feriers")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"idJourFerie\": \"SAMPLE_TEXT\"" + ", \"libelle\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_jourFerier_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created jourFerier").get("${new_jourFerier_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created jourFerier").delete("${new_jourFerier_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the JourFerier entity").exec(scn);
