...
public class DispositifSignatureGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all dispositifSignatures")
                    .get("/api/dispositif-signatures")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dispositifSignature")
                        .post("/api/dispositif-signatures")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"idDispositif\": \"SAMPLE_TEXT\"" + ", \"libelle\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dispositifSignature_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created dispositifSignature").get("${new_dispositifSignature_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created dispositifSignature").delete("${new_dispositifSignature_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DispositifSignature entity").exec(scn);
