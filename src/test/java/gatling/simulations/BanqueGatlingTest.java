 ...
 */
public class BanqueGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all banques").get("/api/banques").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new banque")
                        .post("/api/banques")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"idBanque\": \"SAMPLE_TEXT\"" +
                                ", \"code\": \"SAMPLE_TEXT\"" +
                                ", \"devise\": \"XOF\"" +
                                ", \"langue\": \"FR\"" +
                                ", \"logo\": \"SAMPLE_TEXT\"" +
                                ", \"codeSwift\": \"SAMPLE_TEXT\"" +
                                ", \"codeIban\": \"SAMPLE_TEXT\"" +
                                ", \"codePays\": \"SAMPLE_TEXT\"" +
                                ", \"fuseauHoraire\": \"SAMPLE_TEXT\"" +
                                ", \"cutOffTime\": \"SAMPLE_TEXT\"" +
                                ", \"codeParticipant\": \"SAMPLE_TEXT\"" +
                                ", \"libelleParticipant\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_banque_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created banque").get("${new_banque_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created banque").delete("${new_banque_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Banque entity").exec(scn);
