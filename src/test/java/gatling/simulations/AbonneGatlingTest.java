 ...
 */
public class AbonneGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all abonnes").get("/api/abonnes").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new abonne")
                        .post("/api/abonnes")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"indiceClient\": \"SAMPLE_TEXT\"" +
                                ", \"nomAbonne\": \"SAMPLE_TEXT\"" +
                                ", \"prenomAbonne\": \"SAMPLE_TEXT\"" +
                                ", \"telephone\": \"SAMPLE_TEXT\"" +
                                ", \"email\": \"SAMPLE_TEXT\"" +
                                ", \"typePieceIdentite\": \"CNI\"" +
                                ", \"numPieceIdentite\": \"SAMPLE_TEXT\"" +
                                ", \"statut\": \"DESACTIVE\"" +
                                ", \"identifiant\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_abonne_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created abonne").get("${new_abonne_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created abonne").delete("${new_abonne_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Abonne entity").exec(scn);
