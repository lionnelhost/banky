...
public class ParticipantGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all participants").get("/api/participants").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created participant").get("${new_participant_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created participant").delete("${new_participant_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
