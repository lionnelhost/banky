...
public class ContratAbonnementGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all contratAbonnements").get("/api/contrat-abonnements").headers(headersHttpAuthenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new contratAbonnement")
                        .post("/api/contrat-abonnements")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"idContrat\": \"SAMPLE_TEXT\"" + ", \"idAbonne\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_contratAbonnement_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created contratAbonnement").get("${new_contratAbonnement_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created contratAbonnement").delete("${new_contratAbonnement_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ContratAbonnement entity").exec(scn);
