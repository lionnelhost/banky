...
public class ParametrageGlobalGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all parametrageGlobals").get("/api/parametrage-globals").headers(headersHttpAuthenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new parametrageGlobal")
                        .post("/api/parametrage-globals")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"idParamGlobal\": \"SAMPLE_TEXT\"" +
                                ", \"codeParam\": \"SAMPLE_TEXT\"" +
                                ", \"typeParam\": \"SAMPLE_TEXT\"" +
                                ", \"valeur\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_parametrageGlobal_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created parametrageGlobal").get("${new_parametrageGlobal_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created parametrageGlobal").delete("${new_parametrageGlobal_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ParametrageGlobal entity").exec(scn);
