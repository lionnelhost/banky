 ...
 */
public class TypeContratGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeContrats").get("/api/type-contrats").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeContrat")
                        .post("/api/type-contrats")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"idTypeContrat\": \"SAMPLE_TEXT\"" + ", \"libelle\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeContrat_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeContrat").get("${new_typeContrat_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created typeContrat").delete("${new_typeContrat_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeContrat entity").exec(scn);
