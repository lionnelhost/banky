...
public class MessageErreurGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all messageErreurs").get("/api/message-erreurs").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new messageErreur")
                        .post("/api/message-erreurs")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"idMessageErreur\": \"SAMPLE_TEXT\"" +
                                ", \"codeErreur\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_messageErreur_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created messageErreur").get("${new_messageErreur_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created messageErreur").delete("${new_messageErreur_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MessageErreur entity").exec(scn);
