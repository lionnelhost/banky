...
/**
 * Performance test for the ContratAbonnementCompte entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.7.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class ContratAbonnementCompteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all contratAbonnementComptes")
                    .get("/api/contrat-abonnement-comptes")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new contratAbonnementCompte")
                        .post("/api/contrat-abonnement-comptes")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"idContrat\": \"SAMPLE_TEXT\"" +
                                ", \"idAbonne\": \"SAMPLE_TEXT\"" +
                                ", \"idCompteBancaire\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_contratAbonnementCompte_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created contratAbonnementCompte")
                            .get("${new_contratAbonnementCompte_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created contratAbonnementCompte")
                        .delete("${new_contratAbonnementCompte_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ContratAbonnementCompte entity").exec(scn);
