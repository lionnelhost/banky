...
/**
 * Performance test for the ParametrageNotification entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.7.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class ParametrageNotificationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all parametrageNotifications")
                    .get("/api/parametrage-notifications")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new parametrageNotification")
                        .post("/api/parametrage-notifications")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"idParamNotif\": \"SAMPLE_TEXT\"" +
                                ", \"objetNotif\": \"SAMPLE_TEXT\"" +
                                ", \"typeNotif\": \"SAMPLE_TEXT\"" +
                                ", \"contenu\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_parametrageNotification_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created parametrageNotification")
                            .get("${new_parametrageNotification_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created parametrageNotification")
                        .delete("${new_parametrageNotification_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ParametrageNotification entity").exec(scn);
