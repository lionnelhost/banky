 ...
 */
public class CanalGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all canals").get("/api/canals").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new canal")
                        .post("/api/canals")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"idCanal\": \"SAMPLE_TEXT\"" + ", \"libelle\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_canal_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created canal").get("${new_canal_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created canal").delete("${new_canal_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Canal entity").exec(scn);
