...
/**
 * Performance test for the VariableNotification entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.7.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class VariableNotificationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all variableNotifications")
                    .get("/api/variable-notifications")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new variableNotification")
                        .post("/api/variable-notifications")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"idVarNotification\": \"SAMPLE_TEXT\"" +
                                ", \"codeVariable\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_variableNotification_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created variableNotification").get("${new_variableNotification_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created variableNotification").delete("${new_variableNotification_url}").headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the VariableNotification entity").exec(scn);
