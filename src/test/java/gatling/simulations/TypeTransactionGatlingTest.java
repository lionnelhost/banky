...
public class TypeTransactionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeTransactions").get("/api/type-transactions").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeTransaction")
                        .post("/api/type-transactions")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"idTypeTransaction\": \"SAMPLE_TEXT\"" + ", \"libelle\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeTransaction_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeTransaction").get("${new_typeTransaction_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created typeTransaction").delete("${new_typeTransaction_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeTransaction entity").exec(scn);
