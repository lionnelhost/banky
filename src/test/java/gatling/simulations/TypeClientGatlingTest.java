...
public class TypeClientGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeClients").get("/api/type-clients").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeClient")
                        .post("/api/type-clients")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"idTypeClient\": \"SAMPLE_TEXT\"" + ", \"libelle\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeClient_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeClient").get("${new_typeClient_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created typeClient").delete("${new_typeClient_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeClient entity").exec(scn);
