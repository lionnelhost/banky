 ...
 */
public class CompteBancaireGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all compteBancaires").get("/api/compte-bancaires").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new compteBancaire")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_compteBancaire_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created compteBancaire").get("${new_compteBancaire_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created compteBancaire").delete("${new_compteBancaire_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CompteBancaire entity").exec(scn);
